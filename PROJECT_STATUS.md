...
- **Phase 8** — CSV export (revised scope: CSV only, no PDF/Excel, no new API routes) plus a multi-select enhancement: per-row checkboxes, select-all, live selected count, "Export Selected CSV" alongside the original "Export CSV" (all-filtered). Two export sections on `(admin)/reports`: All Interns and Leaderboard Rankings, each with their own search/department/Top-10 filters, exporting Rank/Name/Email/Department/Score/Status. **Implemented and build-verified — not yet committed** (per instruction, awaiting review).
